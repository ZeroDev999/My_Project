    ...
    if (empty($username) || empty($email) || empty($password) || empty($first_name) || empty($last_name)) {
        ...
        try {
            ...
            if ($stmt->fetch()) {
                ...
                // Insert user
                $stmt = $db->prepare("INSERT INTO users (username, email, password_hash, first_name, last_name, role, email_verification_token) VALUES (?, ?, ?, ?, ?, ?, ?)");
                $stmt->execute([$username, $email, $password_hash, $first_name, $last_name, $role, $verification_token]);
                
                // Send verification email (simplified - in production, use proper email service)
                $verification_link = BASE_URL . "auth/verify-email.php?token=" . $verification_token;
